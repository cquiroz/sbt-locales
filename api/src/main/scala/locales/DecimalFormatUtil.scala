...
object DecimalFormatUtil {
  ...
  def lastIndexOfPattern(pattern: String, symbol: Char): Int = {
    ...
    allPatternChars.filterNot(_ == symbol).map(c => pattern.lastIndexOf(c.toInt, max)).max
  }
  ...
  def groupingCount(pattern: String): Int = {
    ...
    val decIndex           = pat.lastIndexOf(PatternCharDecimalSeparator.toInt)
    val actualDecIndex     = if (decIndex < 0) pat.length - 1 else decIndex - 1
    val lastSeparatorIndex = pat.lastIndexOf(PatternCharGroupingSeparator.toInt, actualDecIndex)
    if (pat.isEmpty) 3
  ...
  private def countMinimum(pattern: String, c: Char, trailingCount: Boolean): Option[Int] =
  ...
  def toParsedPattern(pattern: String): ParsedPattern = {
    ...
    val minIntegerDigits: Option[Int] = {
      ...
      exponentMin
        .orElse(count)
        .orElse(
          if (minFractionDigits.isDefined) None else Some(1)
        ) // Default to 1 if no min fraction or min count
    }
